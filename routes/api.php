...
Route::group(['middleware' => ['auth:sanctum']], function () {
    ...
    Route::middleware('adminRoleRestriction')->get('/management/account', [AccountManagementController::class, 'adminList']);

    // 新規管理者アカウント(owner, counter, kitchen)発行
    Route::middleware('adminRoleRestriction')->post('/management/account', [AccountManagementController::class, 'adminSignUp']);

    // 管理者アカウント(owner, counter, kitchen)削除
    Route::middleware('adminRoleRestriction')->delete('/management/account/{admin_id}', [AccountManagementController::class, 'adminDelete']);
    ...
    // メニューに料理を追加(新規作成)
    Route::middleware('adminRoleRestriction')->post('/management/dish/byRestaurant/{restaurant_id}', [DishManagementController::class, 'dishCreate']);

    // 料理メニューの一覧取得
    Route::get('/management/dish/byRestaurant/{restaurant_id}', [DishManagementController::class, 'dishList']);

    // 料理の削除
    Route::middleware('adminRoleRestriction')->delete('/management/dish/{dish_id}', [DishManagementController::class, 'dishDelete']);
    ...
    Route::middleware('adminRoleRestriction')->get('/management/restaurant', [RestaurantManagementController::class, 'restaurantList']);

    // 新規店舗登録
    Route::middleware('adminRoleRestriction')->post('/management/restaurant', [RestaurantManagementController::class, 'restaurantSignUp']);

    // 店舗の削除
    Route::middleware('adminRoleRestriction')->delete('/management/restaurant/{restaurant_id}', [RestaurantManagementController::class, 'restaurantDelete']);

    // 指定した店舗情報取得
    Route::get('/management/restaurant/{restaurant_id}', [RestaurantManagementController::class, 'restaurantGet']);

    // 店舗情報編集
    Route::middleware('adminRoleRestriction')->put('/management/restaurant/{restaurant_id}', [RestaurantManagementController::class, 'restaurantModify']);

    // 座席の追加
    Route::middleware('adminRoleRestriction')->post('/management/seat/byRestaurant/{restaurant_id}', [SeatManagementController::class, 'seatAdd']);

    // 指定した店舗の座席情報一覧を取得
    Route::get('/management/seat/byRestaurant/{restaurant_id}', [SeatManagementController::class, 'seatList']);

    // 座席の削除
    Route::middleware('adminRoleRestriction')->delete('/management/seat/{seat_id}', [SeatManagementController::class, 'seatDelete']);
    ...
    // 新規店舗の座席一括登録
    Route::middleware('adminRoleRestriction')->post('/management/seat/byRestaurant/{restaurant_id}/{seat_num}', [SeatManagementController::class, 'multipleSeatAdd']);
});
